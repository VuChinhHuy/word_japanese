...
import ListWord from './list_word' // Thêm import này
...
export function LessonContent() {
  ...
  const [showWordList, setShowWordList] = useState(false); // Thêm state này
  ...
  return <>
    {!showWordList ? <div className="w-full flex flex-col max-w-4xl p-4 md:p-6 lg:p-8 h-full">
      <div className='flex flex-none flex-row items-center justify-between mb-4'>
        ...
        <div className='relative flex items-center'> {/* Thêm flex và items-center */}
          {/* Thêm nút hiển thị danh sách từ */}
          <button
            onClick={() => setShowWordList(!showWordList)}
            type="button"
            className='mr-2 h-12 w-12 bg-teal-900/20 dark:bg-slate-400/10 text-teal-950 dark:text-teal-200 rounded-full flex items-center justify-center hover:bg-slate-400/20 dark:hover:bg-slate-200/20 transition-colors'
          >
